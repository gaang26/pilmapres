<div class="row-fluid margin-top-20">
    <div class="span8 offset2">
        <div class="well well-white bordered-dashed-1 bordered-radius-1">
            <h5>JADWAL KEGIATAN PROGRAM PEMILIHAN MAHASISWA BERPRESTASI TINGKAT NASIONAL TAHUN 2016</h5>
            <table class="table table-bordered table-striped">
                <tr>
                    <th>
                        Kegiatan
                    </th>
                    <th>
                        Waktu
                    </th>
                </tr>
                <tr>
                    <td width="60%">
                        Pengiriman data calon Mawapres & berkas elektronik
                    </td>
                    <td>
                        25 Mei 2016 - 5 Juni 2016
                    </td>
                </tr>
                <tr>
                    <td>
                        Penilaian Tahap Pertama Oleh Tim Juri
                    </td>
                    <td>
                        6 - 20 Juni 2016
                    </td>
                </tr>
                <tr>
                    <td>
                        Pengumuman & Undangan untuk Finalis Mawapres
                    </td>
                    <td>
                        30 Juni 2016
                    </td>
                </tr>
                <tr>
                    <td>
                        Masukan publik
                    </td>
                    <td>
                        11 - 27 Juli 2016
                    </td>
                </tr>
                <tr>
                    <td>
                        Penilaian Nasional (Tahap Akhir)
                    </td>
                    <td>
                        14 - 17 Agustus 2016
                    </td>
                </tr>
                <tr>
                    <td>
                        Penganugerahan Mahasiswa Berprestasi
                    </td>
                    <td>
                        17 Agustuss 2016
                    </td>
                </tr>
            </table>

            <small>* Jadwal dapat berubah sewaktu-waktu, dan akan selalu terupdate di laman ini.</small>
        </div>
    </div>
</div>

<div class="row-fluid">
    <div class="span8 offset2">
        <div class="well bordered-dashed-1">
            <h3>Informasi dan bantuan silahkan hubungi</h3>
            <h4><i class="icon-envelope-alt"></i> mawapres.dikti@gmail.com</h4>
        </div>
    </div>
</div>
